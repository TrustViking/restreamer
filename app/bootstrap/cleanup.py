...
_PROTECTED_LOG_FILENAMES: frozenset[str] = frozenset()
...
def run_daily_cleanup(
    *,
    ...
    legacy_marker: Path = project_root / "logs" / _MARKER_FILENAME
    marker_path: Path = project_root / "state" / _MARKER_FILENAME
    if legacy_marker.exists() and not marker_path.exists():
        marker_path.parent.mkdir(parents=True, exist_ok=True)
        legacy_marker.rename(marker_path)
